...
- **Paying a non-AUTHORISED invoice.** Payments apply to approved AR/AP invoices only; `DRAFT`/`SUBMITTED` create no journals and cannot take payments. Approve first: `POST /Invoices/{InvoiceID}` with `{"Status": "AUTHORISED"}` (legal from DRAFT and SUBMITTED ONLY; the MCP server cannot do this — raw API only). `PAID` = already fully paid, nothing to do; `VOIDED`/`DELETED` are terminal — cannot be paid or approved. Exact ValidationError text for this rejection is not published — UNVERIFIED — check before relying.
