...
import type { ApiError, Schemas } from "@/lib/api";
import { Card, CardContent } from "@/components/ui/card";
import { Money } from "@/components/money";
import { LoadingStats } from "@/components/loading-state";
import { ErrorState } from "@/components/error-state";

type Summary = Schemas["DashboardSummaryDto"];

interface Stat {
  label: string;
  icon: LucideIcon;
  render: (s: Summary) => React.ReactNode;
}
...
const STATS: Stat[] = [
  ...
  { label: "Sent this month", icon: Send, render: (s) => s.invoicesSentThisMonth },
  { label: "Clients", icon: Users, render: (s) => s.clientsCount },
];

/**
 * Headline numbers for the top of the inbox. Controlled by the surface so it
 * refetches in step with the task list after every resolution.
 */
